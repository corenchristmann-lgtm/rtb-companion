...
export function PlanningScreen({ timer, challenges, focusId, onClearFocus }: Props) {
  ...
  useEffect(() => {
    if (focusId !== null) {
      setExpandedId(focusId); onClearFocus();
      setTimeout(() => { document.getElementById(`ch-${focusId}`)?.scrollIntoView({ behavior: "smooth", block: "start" }); }, 100);
    }
  ...
  return (
    <div ref={scrollRef} className="px-4 pt-6 pb-4 max-w-lg mx-auto">
      ...
      <p className="text-[11px] text-[#7C6FA0] mb-5">Lundi 13 avril · 8 challenges · Liège</p>
              ...
              <div key={ch.id} id={`ch-${ch.id}`}>
                <button onClick={() => { setExpandedId(expanded ? null : ch.id); setDetailTab("info"); }}
                  ...
                  <div className="relative z-10 mt-3 shrink-0">
                    ...
                    {done && <div className="absolute -bottom-0.5 -right-0.5 w-4 h-4 rounded-full bg-emerald-500 flex items-center justify-center"><span className="text-[8px] text-white font-bold">✓</span></div>}
                    {active && <div className="absolute -bottom-0.5 -right-0.5 w-4 h-4 rounded-full bg-[#7A4AED] animate-breathe" />}
                  </div>
                  <div className={`flex-1 rounded-2xl border p-3.5 my-0.5 transition-all ${
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-[11px] tabular-nums text-[#7C6FA0] font-semibold">{ch.start_time}</span>
                    ...
                    <p className="text-[11px] text-[#7C6FA0] mt-0.5">{ch.format}</p>
                      ...
                      {(["info", "check"] as const).map((t) => (
                        <button key={t} onClick={() => setDetailTab(t)}
                          className={`flex-1 py-1.5 text-[11px] font-semibold rounded-lg transition-colors ${
                            detailTab === t ? "bg-white text-[#1A1035] shadow-sm" : "text-[#7C6FA0]"
                          }`}>{t === "info" ? "Infos" : "Checklist"}</button>
                      ))}
                    </div>
      ...
      <div className="flex items-center gap-3 mt-3 ml-1">
        <div className="w-[38px] flex justify-center"><div className="w-2.5 h-2.5 rounded-full bg-[#FFE3E8]" /></div>
        <p className="text-xs text-[#7C6FA0]">18h00 · Verre de clôture</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="space-y-2.5 text-[13px]">
        <div><p className="text-[10px] font-bold text-[#7C6FA0] uppercase tracking-widest">Lieu</p><p className="font-medium text-[#1A1035]">{ch.location}</p><p className="text-[#7C6FA0] text-xs">{ch.address}</p></div>
        <div><p className="text-[10px] font-bold text-[#7C6FA0] uppercase tracking-widest">Jury</p><p className="text-[#1A1035]/60">{ch.jury.join(" · ")}</p></div>
        {ch.contact_name && (
          <div className="flex items-center justify-between">
            <div><p className="text-[10px] font-bold text-[#7C6FA0] uppercase tracking-widest">Contact</p><p className="text-[#1A1035]">{ch.contact_name}</p></div>
            {ch.contact_phone && (
              <a href={`tel:${ch.contact_phone.replace(/\s/g, "")}`}
                className="text-xs text-white bg-[#7A4AED] px-3 py-1.5 rounded-lg font-semibold min-h-[44px] flex items-center shadow-sm">Appeler</a>
            )}
          </div>
        )}
      </div>
