...
export function ScoreScreen({ timer }: Props) {
  ...
  return (
    <div className="px-4 pt-6 pb-4 max-w-lg mx-auto space-y-4">
          ...
          return (
            <button key={c.id} onClick={() => setSelectedChallenge(c.id)}
              className={`shrink-0 flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold transition-all ${
                isSelected
                  ? "bg-[#7A4AED] text-white shadow-lg shadow-[#7A4AED]/25"
                  : hasNotes
                  ? "bg-emerald-50 border border-emerald-200 text-emerald-700"
                  : "bg-white border border-[#E8E2F4] text-muted-foreground shadow-sm"
              }`}>
      ...
      <p className="text-[11px] text-muted-foreground px-1">{ch.company} · {ch.format}</p>

      {/* Score cards */}
      <div className="space-y-2.5">
        {PROJECTS.map((p) => {
          const note = notes.find((n) => n.project_id === p.id && n.challenge_id === selectedChallenge) ?? null;
          return <ScoreCard key={p.id} project={p} challengeId={selectedChallenge} note={note} onUpdate={upsertNote} />;
        })}
      </div>
    </div>
  );
  ...
  return (
    <div className="rounded-2xl border border-[#E8E2F4] bg-white shadow-sm overflow-hidden">
      ...
      <button onClick={() => setExpanded(!expanded)}
        className="w-full px-4 py-1.5 text-[10px] text-muted-foreground font-semibold border-t border-[#E8E2F4] bg-[#FAFAFA] transition-colors">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              ...
              <input type="text" value={note?.strength ?? ""}
                ...
                placeholder="Force…"
                className="w-full h-8 px-2.5 bg-emerald-50 rounded-lg text-xs placeholder:text-emerald-300 focus:outline-none focus:ring-1 focus:ring-emerald-300" />
            </div>
            <div>
              ...
              <input type="text" value={note?.improvement ?? ""}
                ...
                placeholder="Axe…"
                className="w-full h-8 px-2.5 bg-[#FFE3E8] rounded-lg text-xs placeholder:text-[#F46277]/30 focus:outline-none focus:ring-1 focus:ring-[#F46277]/30" />
            </div>
          </div>
        </div>
      )}
