  ...
  return (
    <div className="px-4 pt-6 pb-4 max-w-lg mx-auto space-y-3">
      ...
      {PROJECTS.map((p) => {
        ...
        return (
          <div key={p.id} className="rounded-2xl border border-[#E8E2F4] bg-white shadow-sm overflow-hidden">
            ...
            {open && (
              <div className="px-4 pb-4 pt-2 border-t border-[#E8E2F4] animate-slide-up">
                ...
                {pNotes.some((n) => n.strength) && (
                  <div className="mt-3">
                    <p className="text-[9px] font-bold text-emerald-500 uppercase tracking-widest mb-1">Points forts</p>
                    <div className="flex flex-wrap gap-1">
                      {pNotes.filter((n) => n.strength).map((n) => (
                        <span key={n.challenge_id} className="text-[10px] bg-emerald-50 text-emerald-600 px-1.5 py-0.5 rounded-md">{n.strength}</span>
                      ))}
                    </div>
                  </div>
                )}
